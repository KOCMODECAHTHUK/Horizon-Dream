...
import {
  ...
  lightenColor,
  darkenColor,
  projectPoint as projectPointUtil,
  unprojectToGroundPlane as unprojectToGroundPlaneUtil,
} from './SupercruiseMapUtils';
...
export class SupercruiseMapCanvas extends Component {
  ...
  draw() {
    ...
    for (const item of drawItems) {
      ...
      if (isOurShuttle) {
        ...
        if (velMag > 0.5) {
          const velScale = 3;
          const velEnd = this.projectPoint(worldX + velX2 * velScale, worldY + velY2 * velScale, worldZ + velZ2 * velScale);
        ...
        if (shuttleThrust > 0) {
          ...
          if (thrustMag > 0.001) {
            ...
            const thrustEnd = this.projectPoint(worldX + tx * thrustScale, worldY + ty * thrustScale, worldZ + tz * thrustScale);
  ...
  drawSphereCompass(ctx, cx, cy, radius, cameraYaw, cameraPitch, shipHeading, shipHeadingPitch, shuttleThrust, shuttleAngle, velMag, velX, velY, velZ) {
    ...
    if (velMag > 0.5) {
      ...
      this.drawArrowAsPyramid(ctx, projectWorld, velDir, '#00ffff', 0.06, 0.34, false);
    }
  ...
  drawArrowAsPyramid(ctx, projectWorld, tipDir, color, baseSize = 0.06, height = 0.2, baseAtCenter = false) {
    ...
    let perp1, perp2;
    if (Math.abs(tipDir.x) < 0.9) { perp1 = { x: 0, y: -tipDir.z, z: tipDir.y }; }
    else { perp1 = { x: -tipDir.z, y: 0, z: tipDir.x }; }

    const len1 = Math.sqrt(perp1.x ** 2 + perp1.y ** 2 + perp1.z ** 2);
    ...
    for (let i = 0; i < baseSides; i++) {
      ...
      const baseX = perp1.x * Math.cos(angle) * baseSize + perp2.x * Math.sin(angle) * baseSize;
      const baseY = perp1.y * Math.cos(angle) * baseSize + perp2.y * Math.sin(angle) * baseSize;
      const baseZ = perp1.z * Math.cos(angle) * baseSize + perp2.z * Math.sin(angle) * baseSize;
  ...
  handleMouseMove = (e) => {
    if (this.adjustingZ) {
      ...
      const zScale = 2 / (this.props.zoomScale || 1);
    ...
    if (e.button === 0) {
      ...
      if (this.adjustingZ) {
        ...
        if (this.props.onMapClick) {
          this.props.onMapClick(this.adjustWorldX, this.adjustWorldY, 'right', false, null, this.adjustCurrentZ);
        }
        this.localPendingX = null;
        this.localPendingY = null;
        this.localPendingZ = null;
        ...
        if (this.props.onMapClick) {
          this.props.onMapClick(0, 0, 'cancel', true, null, 0);
        }
