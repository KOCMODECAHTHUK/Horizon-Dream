/obj/machinery/atmospherics/components/unary/fueled_engine_heater
	name = "Fueled Engine Heater"
	desc = "A small heater that is designed to contain a combustible gas for it's connected engine."
	icon = '_horizon/code/modules/shuttle/icons/shuttle.dmi'
	icon_state = "heater"
...
/obj/machinery/atmospherics/components/unary/fueled_engine_heater/Initialize(mapload)
	air_contents = new(internal_volume)
	. = ..()
	register_context()

/obj/machinery/atmospherics/components/unary/fueled_engine_heater/post_machine_initialize()
	. = ..()
	try_link_engine()

/obj/machinery/atmospherics/components/unary/fueled_engine_heater/proc/try_link_engine()
	...
	if(!candidate_engine || candidate_engine.dir != dir)
		return FALSE
	on_engine_link(candidate_engine)

/// Called when the engine is linked to a heater; must call parent first.
/obj/machinery/atmospherics/components/unary/fueled_engine_heater/proc/on_engine_link(obj/machinery/power/shuttle_engine/ship/fueled/connecting)
...
/obj/machinery/atmospherics/components/unary/fueled_engine_heater/proc/on_engine_unlink()
	SHOULD_CALL_PARENT(TRUE)
	if(!connected_engine)
		return
...
/obj/machinery/atmospherics/components/unary/fueled_engine_heater/Destroy()
	if(connected_engine)
		on_engine_unlink()
	return ..()

/obj/machinery/atmospherics/components/unary/fueled_engine_heater/set_init_directions()
	initialize_directions = dir

/obj/machinery/atmospherics/components/unary/fueled_engine_heater/proc/get_dump_air_mixture()
...
/obj/machinery/atmospherics/components/unary/fueled_engine_heater/proc/dump_air_contents()
	var/datum/gas_mixture/dump_into = get_dump_air_mixture()
	dump_into.merge(air_contents.remove_ratio(1))

/obj/machinery/atmospherics/components/unary/fueled_engine_heater/proc/filter_air_contents()
	var/datum/gas_mixture/dump_into = get_dump_air_mixture()
	if(purge_mode)
...
/obj/machinery/atmospherics/components/unary/fueled_engine_heater/proc/input_air_contents()
	if(!length(nodes) || !nodes[1])
		return
...
/obj/machinery/atmospherics/components/unary/fueled_engine_heater/process_atmos()
	if(!connected_engine)
		return PROCESS_KILL
...
/obj/machinery/atmospherics/components/unary/fueled_engine_heater/atom_deconstruct(disassembled)
...
/obj/machinery/atmospherics/components/unary/fueled_engine_heater/screwdriver_act(mob/living/user, obj/item/tool)
	var/initial = initial(icon_state)
	if(default_deconstruction_screwdriver(user, "[initial]_open", initial, tool))
		return TRUE

/obj/machinery/atmospherics/components/unary/fueled_engine_heater/crowbar_act(mob/living/user, obj/item/tool)
	. = ..()
	if(default_deconstruction_crowbar(tool))
		return TRUE

/obj/machinery/atmospherics/components/unary/fueled_engine_heater/multitool_act(mob/living/user, obj/item/tool)
	if(!panel_open)
		return FALSE
...
/obj/machinery/atmospherics/components/unary/fueled_engine_heater/multitool_act_secondary(mob/living/user, obj/item/tool)
	if(!panel_open)
		return FALSE
...
/*
/obj/machinery/atmospherics/components/unary/fueled_engine_heater/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	if(panel_open)
