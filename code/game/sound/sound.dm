...
/proc/get_channel_info(channel)
	switch(channel)
		...
		if(CHANNEL_ADMIN)
			return list("Admin MIDI", "Sound of Admin-played music.")
		if(CHANNEL_VOX)
			return list("AI Vox & Blips", "AI VOX and the sound of AIs speaking over the radio.")
		if(CHANNEL_ANNOUNCEMENTS)
			return list("Announcements", "The sound of reports from the Captain/Syndicate/Central Command.")
		...
		if(CHANNEL_WEATHER)
			return list("Weather", "Looping noise of Lavaland, Icemoon, and Void environments.")
		if(CHANNEL_MACHINERY)
			return list("Machinery", "Airlocks, Buttons, Remotes, and all other machines.")
		if(CHANNEL_INSTRUMENTS)
			return list("Player Instruments", "All instruments played by non-silicon mobs.")
		if(CHANNEL_INSTRUMENTS_ROBOT)
			return list("Robot Instruments", "All instruments played by silicon mobs.")
		if(CHANNEL_MOB_SOUNDS)
			return list("Mob Sounds", "Radio noises, chittering, and creature sounds.")
		if(CHANNEL_PRUDE)
			return list("Prude Sounds", "Farting and gas sounds.")
		if(CHANNEL_SQUEAK)
		...
		if(CHANNEL_DELTA_SIRENS)
			return list("Sirens", "The sirens that blare during a Delta Alert.")
		if(CHANNEL_ADMIN_SOUNDS)
			return list("Admin Sounds", "Used for fax requests, prayers, and bwoinks. Admin-only.")
		if(CHANNEL_SHUTTLES)
			return list("Shuttles", "The sound of shuttles booting, docking, and departing.")
